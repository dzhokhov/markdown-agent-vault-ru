...
- текущая картина — если объявлена в `README.md` или `context.md`, это актуальный пересобираемый слой проекта. Парковка не заменяет её, но если при стоп-кадре стало понятно, что текущий расклад изменился, обнови текущую картину до записи в `log.md`. См. [Память vault](../../meta/rules/vault-memory.md).
...
Если у проекта объявлена текущая картина и прерывание меняет ответ «что сейчас главное», пересобери её после `plan.md` и до `log.md`.
...
Пример:
```
- 2026-03-07: ⏸️ BLOCKED PAUSED | Задача: unit-экономика новых тарифов.
  Остановился на: собрал все метрики кроме себестоимости транзакции.
  Причина: нет данных по себестоимости, нужны от финансов.
  Действие: написал @Иванову в Slack, тред #pricing.
  Вернуться когда: получу цифры или через 3 дня (пнуть 2026-03-10).
```
...
Пример карточки блокера в `plan.md`:
```markdown
### B1 — Себестоимость транзакции от финансов
- Открыт: 2026-03-07
- Тип: delegated-work
- Ждём: цифры по себестоимости, чтобы пересчитать unit-экономику
- Блокирует: M3
- Моё действие: active-nudge (пнуть @Иванова 2026-03-10)
- Условие возврата: получены цифры в треде #pricing
- Fallback: брать оценку из 2025-Q4 отчёта, помечать как «приближение»
- Stale check: 7 дней
```

Пример параллельной строки в `delegations/ivanov.md`:
```markdown
- [ ] 2026-03-07 → собрать себестоимость транзакций по тарифам [SHOP-2251](https://tracker.example.com/issue/SHOP-2251) • due: 2026-03-10 • [plan](../../../projects/2026-acme-shop-pricing/plan.md#b1)
```
...
> Припарковался. 2026-acme-shop-pricing — log.md, делегирование в ops/acme-shop/delegations/ivanov.md.
> Точка остановки: собрал все метрики кроме себестоимости.
> Триггер возврата: @Иванов присылает цифры или 2026-03-10.
> Можем переключаться.
