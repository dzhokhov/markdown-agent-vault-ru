...
- При восстановлении контекста проекта порядок чтения служебных файлов: **README → plan (включая §Blockers) → context → текущая картина, если объявлена → tasks → log** (медленные слои опережают быстрые, как в [write-protocol.md §5](../../meta/rules/write-protocol.md)).
...
1. ⏸️ [2026-03-07] acme-shop — unit-экономика новых тарифов
   Ждём: ответ от @Иванова по себестоимости

2. 🔀 [2026-03-05] personal-blog — контент-план для Instagram
   Делегировано: @Петрова, ожидаем драфт

3. ⏹️ [2026-03-06] japan-trip — бронирование Ryokan в Hakone
...
Если пользователь уже указал конкретный проект или задачу в своём запросе (например, «resume acme-shop» или «продолжи с unit-экономикой») — не показывай полный список, а сразу переходи к шагу 3 для этой задачи.
...
4. Текущая картина, если она объявлена в `README.md` или `context.md` — что сейчас главное, что устарело, что опровергнуто, какие вопросы открыты
5. `01_now/projects/<project>/tasks.md` — execution queue с маркерами прерываний
6. `01_now/projects/<project>/log.md` — последние 5–10 записей для понимания хронологии
7. Для каждого упомянутого в парковке сотрудника — **passive recall** по `01_now/ops/<contour>/delegations/<person-slug>.md` секция `## Active`. Покажи пользователю, что ещё висит на этом человеке, чтобы не терять смежные блокеры. Для каждого блокера в `plan.md §Blockers` типа `delegated-work` — убедись, что parallel-строка в delegations ещё актуальна
8. Если парковка относится к личному обязательству — `01_now/personal/tasks.md` §Hard
...
> Текущая картина: [если есть — 1 строка, что сейчас главное].
