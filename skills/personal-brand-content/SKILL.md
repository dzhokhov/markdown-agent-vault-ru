...
Создание и адаптация экспертного контента для личного бренда в нише **AI-трансформация / AdTech**. Скилл помогает: генерировать контент по идее из backlog, адаптировать мастер-версию под конкретную платформу, поддерживать единый голос автора.
...
1. `01_now/projects/2026-personal-brand/context.md` — ниша, аудитория, столпы, форматы
2. `01_now/projects/2026-personal-brand/pipeline.md` — методология конвейера
3. `01_now/projects/2026-personal-brand/platforms/{platform}.md` — профиль целевой платформы
...
**Запрещено:**
- Вода, штампы, канцелярит (прогнать через скилл финальной вычитки, если он задан в `meta/config/agent-profile.md`)
...
- Если в `meta/config/agent-profile.md` задан скилл финальной вычитки — прогони текст через него; если не задан — вычитай по `meta/rules/` и добавь пометку «пропущено: нет скилла финальной вычитки»
...
- Скилл финальной вычитки (если задан в `meta/config/agent-profile.md`) — финальная чистка текстов
- Скилл отбора сигналов (`skills/signal-intelligence/`, если есть в хранилище) — источник тем из AI-сигналов
...
- `references/telegram-best-practices.md`
