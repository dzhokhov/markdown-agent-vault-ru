...
Сначала сохрани состояние в durable source of truth, потом переводи работу в новый чат. Не делай длинный пересказ беседы, если уже есть `log.md`, `tasks.md`, `context.md`, текущая картина, research-note, spec или другой канонический артефакт.
...
1. существующий проектный контур — `README.md` → `plan.md` → `context.md` → текущая картина, если объявлена → `tasks.md` → `log.md`;
...
- в следующем чате рекомендуй вход через `resume` — он сам перечитает README → plan → context → текущую картину при наличии → tasks → log и сделает passive recall по delegations.
...
- Source of truth: <1-4 файла — в порядке plan → context → текущая картина → tasks → log>
