...
Пример ниже вымышленный: интервью с небольшой студией «Пример».

| Уровень | Формулировка | Вымышленный пример |
|---------|-------------|-------------------|
| **Experiment** | «Мы сделали это…» | Интервью о работе с заявками |
| **Fact** | Наблюдение без интерпретации | «Сотрудник записывает заявки в таблицу и проверяет её раз в день» |
| **Insight** | Интерпретация одного или нескольких фактов | «Редкая проверка таблицы может задерживать ответы; гипотезу нужно проверить» |
| **Recommendation** | Действие на основе инсайта | «На следующем интервью измерить время до первого ответа» |
...
|-----------|------|--------|--------|
| Полевые заметки (пачка наблюдений из одной встречи) | `03_knowledge/field-notes-{тема}-{дата}.md` | Markdown с frontmatter `confidence: single-source` | `field-notes-example-studio-2026-04.md` |
...
| Проектно-специфичные следствия | `01_now/projects/<project>/context.md` | Ссылка на field note + краткое следствие | «Проверить гипотезу о времени ответа» → context.md учебного проекта |
| Кросс-встречный паттерн (когда ≥2 встречи подтвердили) | `03_knowledge/patterns-{домен}.md` или отдельный файл | Markdown с `confidence: emerging` | «Задержка ответа в небольших командах» → отдельный паттерн после независимого подтверждения |
| Открытый вопрос для следующих встреч | `01_now/projects/<project>/plan.md` §Open Questions | Строка с ссылкой на field note | «Задерживаются ли ответы в других студиях?» |
| Задача (нужно что-то сделать) | `tasks.md` соответствующего проекта | Строка задачи | «Подготовить вопросы для следующего интервью» |
...
- Для нового полевого наблюдения предусмотрена маркировка `pattern_confidence: single-source` в протоколе записи
