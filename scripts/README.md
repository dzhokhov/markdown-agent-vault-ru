...
Для дополнительных слов и фрагментов используйте внешний UTF-8 файл: `python3 -B scripts/check_forbidden_markers.py . --markers <локальный-файл>`. Одна буквальная подстрока на строку; приватный список не добавляется в репозиторий. Результат содержит только файл, строку и категорию, без найденного значения. Отсутствие находок не заменяет проверку содержимого перед публикацией.

[Test_checkers](./test_checkers.py) создаёт только искусственные временные файлы внутри тестируемой копии; `TemporaryDirectory` удаляет их после теста. Реальные ключи для проверок не нужны.
