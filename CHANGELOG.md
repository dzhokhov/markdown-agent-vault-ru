...
Первая публичная тестовая версия новой архитектуры. Ветка: `testing/v1.0.0-beta.1`; стабильный выпуск 1.0.0 ещё не объявлен.

- Общие правила и скиллы отделены от четырёх конфигов `meta/config/`; добавлены шаблоны настроек и необязательные модули.
- Добавлен `vault-setup`: до трёх вопросов, создание областей работы, импорт после показа плана и сохранение источников.
- Согласованы признаки первого запуска, отложенная настройка трекера и размещение новых контуров в `01_now/ops/`.
- Разбор встреч разделён на основную инструкцию и пять справочников; обновлены работа с текущей памятью, создание проектов и продолжение сеансов.
- Добавлены требования к понятности инструкций для моделей разной мощности; результаты живых сравнительных прогонов пока не заявляются.
- Очищены личные примеры, исправлены ссылки, добавлены переносимые проверки и регрессионные тесты.
- Инструкции скачивания закреплены за тестовой веткой и проверкой файла `VERSION`.

Подробности, ограничения и сценарий теста: [описание версии](./docs/releases/v1.0.0-beta.1.md).
